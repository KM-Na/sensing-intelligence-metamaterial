...
from blockymetamaterials.geometry import compute_edge_angles, rotation_matrix
from blockymetamaterials.kinematics import block_to_node_kinematics, block_to_dipole_configuration, compute_current_nodes
from blockymetamaterials.utils import BondParams, ControlParams
...
def ligament_strains_linearized(DOFs1: jnp.ndarray, DOFs2: jnp.ndarray, reference_vector: jnp.ndarray = jnp.array([1., 0.])):
    ...
    shear_strain = jnp.cross(reference_vector, dU, axis=-1) / jnp.linalg.norm(reference_vector, axis=-1)**2 \
        - (DOFs2[:, 2] + DOFs1[:, 2])/2
    # print("ligament_strains_linearized : ", axial_strain.shape, shear_strain.shape, dRot.shape)
...
def ligament_energy_linearized(nodal_DOFs: Tuple[jnp.ndarray, jnp.ndarray], reference_vector: jnp.ndarray = jnp.array([1., 0.]), k_stretch=1., k_shear=1., k_rot=1.):
    ...
    axial_strain, shear_strain, dRot = ligament_strains_linearized(*nodal_DOFs, reference_vector=reference_vector)
    l0 = jnp.linalg.norm(reference_vector, axis=-1)
    # print("ligament_energy_linearized : ", axial_strain.shape, shear_strain.shape, dRot.shape)
...
def ligament_strains(DOFs1: jnp.ndarray, DOFs2: jnp.ndarray, reference_vector: jnp.ndarray = jnp.array([1., 0.])):
    ...
    current_bond_angle = jnp.arctan2(*(jnp.roll(current_bond_vector, 1, axis=-1)).T)
    ...
    reference_bond_pushed_angle = jnp.arctan2(*(jnp.roll(reference_bond_pushed, 1, axis=-1)).T)

    axial_strain = (vdot(current_bond_vector, current_bond_vector) / vdot(reference_vector, reference_vector))**0.5 - 1
...
def ligament_energy(nodal_DOFs: Tuple[jnp.ndarray, jnp.ndarray], reference_vector: jnp.ndarray = jnp.array([1., 0.]), k_stretch=1., k_shear=1., k_rot=1.):
    ...
    axial_strain, shear_strain, dRot = ligament_strains(*nodal_DOFs, reference_vector=reference_vector)
    l0 = jnp.linalg.norm(reference_vector, axis=-1)
...
def void_angles(current_block_nodes: jnp.ndarray, bond_connectivity: jnp.ndarray):
    ...
    Args:
        current_block_nodes (jnp.ndarray): array of shape (all_number_of_nodes, 2) defining the current position of the blocks.
    ...
    angles = vmap(lambda bond: compute_edge_angles(current_block_nodes, bond))(bond_connectivity)
...
def edges_distance(edge_1: jnp.ndarray, edge_2: jnp.ndarray):
    """Computes the distance between two edges by projecting the second edge on the first edge.
    ...
    Returns:
        jnp.ndarray: distance between the two edges.
    """

    # Project second edge on first edge
    x0 = edge_1[0]
    x1 = edge_1[1]
    x2 = edge_2[1]
    t = jnp.dot(x2-x0, x1-x0)/jnp.dot(x1-x0, x1-x0)
    return jnp.where(
        (t >= 0) & (t <= 1),
        # Projected point is on the edge
        jnp.sum((x2-x0)**2 - (t*(x1-x0))**2)**0.5,
        jnp.where(
...
def build_void_edge_distance(bond_connectivity: jnp.ndarray):
    ...
    def void_edge_distance(current_block_nodes: jnp.ndarray):
        ...
        Returns:
            jnp.ndarray: array of shape (4*n_bonds,) defining all the pairwise distances between edges connected by the bonds.
        """

        # _, n_nodes_per_block, _ = current_block_nodes.shape
        nodes_1_id = bond_connectivity[:, 0]
        nodes_2_id = bond_connectivity[:, 1]
        # pts1 = current_block_nodes[nodes_1_id//n_nodes_per_block, nodes_1_id % n_nodes_per_block]
        pts1 = current_block_nodes[nodes_1_id]
        # pts1_prev = current_block_nodes[nodes_1_id//n_nodes_per_block, (nodes_1_id-1) % n_nodes_per_block]
        # pts1_next = current_block_nodes[nodes_1_id//n_nodes_per_block, (nodes_1_id+1) % n_nodes_per_block]
        pts1_prev = current_block_nodes[nodes_1_id - 1]
        pts1_next = current_block_nodes[nodes_1_id + 1]

        # pts2 = current_block_nodes[nodes_2_id//n_nodes_per_block, nodes_2_id % n_nodes_per_block]
        pts2 = current_block_nodes[nodes_2_id]
        # pts2_prev = current_block_nodes[nodes_2_id//n_nodes_per_block, (nodes_2_id-1) % n_nodes_per_block]
        # pts2_next = current_block_nodes[nodes_2_id//n_nodes_per_block, (nodes_2_id+1) % n_nodes_per_block]
        pts2_prev = current_block_nodes[nodes_2_id - 1]
        pts2_next = current_block_nodes[nodes_2_id + 1]

        # Distance between edges on one side of the bond
        void_distances1 = jnp.concatenate([
            edges_distance_mapped(
                jnp.concatenate((pts1[:, None], pts1_next[:, None]), axis=1),
                jnp.concatenate((pts2[:, None], pts2_prev[:, None]), axis=1)
            ),  # Distance between second edge and first edge
            edges_distance_mapped(
                jnp.concatenate((pts2[:, None], pts2_prev[:, None]), axis=1),
                jnp.concatenate((pts1[:, None], pts1_next[:, None]), axis=1)
            )  # Distance between first edge and second edge
        ])
        # Distance between edges on the other side of the bond
        void_distances2 = jnp.concatenate([
            edges_distance_mapped(
        ...
        void_distances3 = jnp.concatenate([
            edges_distance_mapped(
                jnp.concatenate((pts1_next[:, None], pts1[:, None]), axis=1),
                jnp.concatenate((pts2_prev[:, None], pts2[:, None]), axis=1)
            ),
            edges_distance_mapped(
                jnp.concatenate((pts2_prev[:, None], pts2[:, None]), axis=1),
                jnp.concatenate((pts1_next[:, None], pts1[:, None]), axis=1)
        ...
        void_distances4 = jnp.concatenate([
            edges_distance_mapped(
                jnp.concatenate((pts1_prev[:, None], pts1[:, None]), axis=1),
                jnp.concatenate((pts2_next[:, None], pts2[:, None]), axis=1)
            ),
            edges_distance_mapped(
                jnp.concatenate((pts2_next[:, None], pts2[:, None]), axis=1),
                jnp.concatenate((pts1_prev[:, None], pts1[:, None]), axis=1)
        ...
        return jnp.concatenate((void_distances1, void_distances2, void_distances3, void_distances4))
...
def contact_energy(current_void_angles: jnp.ndarray, min_angle: jnp.ndarray = jnp.array(0.), cutoff_angle: jnp.ndarray = jnp.array(2.0*jnp.pi/180), k_contact=1.0):
    ...
    energy = jnp.where(
        ...
        jnp.where(
            ...
            k_contact/4 * (cutoff_angle-min_angle)**2 * ((x+1)**-1 - (x-1)**-1 - 2),
            0
        )
    )
    return energy
...
def build_contact_energy(bond_connectivity: jnp.ndarray):
    ...
    def contact_energy_fn(block_displacement: jnp.ndarray, control_params: ControlParams):
        ...
        current_block_nodes = jnp.vstack( compute_current_nodes(block_displacement, control_params) )

        return jnp.sum(contact_energy(current_void_angles=void_angles(current_block_nodes, bond_connectivity), **contact_params._asdict()))
...
def build_strain_energy(bond_connectivity: jnp.ndarray, bond_energy_fn: Callable = ligament_energy_linearized):
    ...
    strain_energy_bonds = strain_energy_bond(bond_connectivity=bond_connectivity, bond_energy_fn=bond_energy_fn)
    ...
    def strain_energy_fn(block_displacement: jnp.ndarray, control_params: ControlParams):
        ...
        node_displacements = jnp.vstack( 
            block_to_node_kinematics(
            block_displacement,
            centroid_node_vectors
        ))
        # node_displacements = node_displacements.reshape((n_blocks * n_nodes_per_block, 3))
...
def constrain_energy(energy_fn: Callable, constrained_kinematics: Callable):
    ...
    def constrained_energy_fn(free_DOFs, t, control_params: ControlParams):
        return energy_fn(
            constrained_kinematics(free_DOFs, t, control_params.constraint_params),
            control_params
        )
...
compute_ligament_strains_history = vmap(compute_ligament_strains, in_axes=(0, None, None, None))
