...
#         jnp.dot(rotation_matrix(block_rotation) - jnp.eye(2), centroid_node_vectors)
...
# block_to_node_kinematics = vmap(vmap(_block_to_node_displacement, in_axes=(None, 0)), in_axes=(0, 0))


@jit
def _single_block_to_node_displacement(block_displacement: jnp.ndarray,
                                       centroid_node_vectors: jnp.ndarray):
    """
    Args:
        block_displacement: shape (3,) = (dx, dy, rotation)
        centroid_node_vectors: shape (n_nodes, 2)

    Returns:
        ndarray: shape (n_nodes, 3) displacement for all nodes of this block
    """
    block_centroid_displacement = block_displacement[:2]
    block_rotation = block_displacement[2]

    R = rotation_matrix(block_rotation)
    aff = R - jnp.eye(2)

    # (n_nodes, 2) @ (2, 2) -> (n_nodes, 2)
    node_disp = centroid_node_vectors @ aff.T
    node_disp = node_disp + block_centroid_displacement[None, :]  # broadcasts over rows

    rot_col = jnp.full((node_disp.shape[0], 1), block_rotation)
    return jnp.hstack([node_disp, rot_col])

def block_to_node_displacement_all(block_displacements: jnp.ndarray,
                                   centroid_node_vectors_list: tuple):
    """
    Args:
        block_displacements: shape (N, 3), one row per block
        centroid_node_vectors_list: tuple of arrays with shape (ni, 2)

    Returns:
        tuple of arrays, each (ni, 3) displacements
    """
    results = []
    for disp, vecs in zip(block_displacements, centroid_node_vectors_list):
        results.append(_single_block_to_node_displacement(disp, vecs))
    results = tuple(results)
    # flatten to a single (sum_ni, 3) array if needed
    offsets = jnp.cumsum(jnp.array([0] + [r.shape[0] for r in results[:-1]]))
    
    # flat = jnp.vstack(results)
    # keep `offsets` if you need to map back per-block
    return results

block_to_node_kinematics = jit(block_to_node_displacement_all)
...
# In previous version, they use DOFsInfo to get the free DOFs,
# but now they use the free DOFs directly because DOFsInfo assumes the blocks have same number of nodes.
...
def block_to_dipole_configuration(block_displacements: jnp.ndarray, block_centroids: jnp.ndarray, dipole_angles: jnp.ndarray, magnetic_block_ids: jnp.ndarray):
    ...
    dipole_locations = block_centroids[magnetic_block_ids] + block_displacements[magnetic_block_ids, :2]
    current_dipole_inplane_angles = block_displacements[magnetic_block_ids, 2] + \
        dipole_angles[:, 0]  # in-plane angle of the dipole
    current_dipole_outofplane_angles = dipole_angles[:, 1]  # out-of-plane angle of the dipole (pitch)
    return jnp.column_stack((dipole_locations, current_dipole_inplane_angles, current_dipole_outofplane_angles))

# The code is for calculating the current node coordinates given the block displacements and geometry parameters.
@jit
def _nodes_for_block(centroid_2d: jnp.ndarray,
                     centroid_node_vecs: jnp.ndarray,
                     node_disp_2d: jnp.ndarray) -> jnp.ndarray:
    """
    centroid_2d: (2,)
    centroid_node_vecs: (ni, 2)
    node_disp_2d: (ni, 2)
    returns: (ni, 2)
    """
    return centroid_2d[None, :] + centroid_node_vecs + node_disp_2d

def compute_current_nodes(block_displacement: jnp.ndarray, control_params):
    # unpack ragged geometry
    block_centroids = control_params.geometrical_params.block_centroids         # tuple of (2,) or ndarray (N,2)
    centroid_node_vectors = control_params.geometrical_params.centroid_node_vectors  # tuple of (ni,2)

    # get per-block node displacements (tuple of (ni,3))
    node_disp_list_3d = block_to_node_kinematics(block_displacement, centroid_node_vectors)

    # keep only x,y per node (still a tuple of ragged arrays)
    node_disp_list_2d = tuple(nd[:, :2] for nd in node_disp_list_3d)

    # normalize centroids to a tuple of (2,)
    if isinstance(block_centroids, jnp.ndarray):
        centroids_tuple = tuple(block_centroids[i] for i in range(block_centroids.shape[0]))
    else:
        centroids_tuple = tuple(block_centroids)

    # per-block node coordinates (tuple of (ni,2))
    current_nodes_list = tuple(
        _nodes_for_block(c, v, d)
        for c, v, d in zip(centroids_tuple, centroid_node_vectors, node_disp_list_2d)
    )
    return current_nodes_list  # tuple of (ni, 2)

def compute_current_nodes_for_plot(block_displacement: jnp.ndarray, block_centroids, centroid_node_vectors):
